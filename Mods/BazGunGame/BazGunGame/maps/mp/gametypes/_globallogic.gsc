...
SetupCallbacks()
{
	...
	level.onForfeit = ::blank; // Lepko -> ::default_onForfeit;
...
menuAutoAssign()
{
	...
	if ( level.teamBased )
	{
		...
		{
			self beginClassChoice(assignment);
			return;
		}
	}
...
beginClassChoice( team )														/* GunGame [WholeFunction] */
{
	...
	self.pers[self.class]["loadout_primary"] = "none";
	self.pers[self.class]["loadout_primary_attachment"] = "none";
	
	self.pers[self.class]["loadout_secondary"] = "none";
	self.pers[self.class]["loadout_secondary_attachment"] = "none";
	
	self.pers[self.class]["loadout_grenade"] = "none";
	
	self.pers[self.class]["loadout_perk1"] = "specialty_null";
	self.pers[self.class]["loadout_perk2"] = "specialty_null";
	self.pers[self.class]["loadout_perk3"] = "specialty_null";
...
musicController()
{
	...
	if ( !level.hardcoreMode && level.mw_music )
...
Callback_StartGameType()
{
	...
	gungame\main::init();														/* GunGame [OneLine] */
...
//	self thread fakeLag();
	...
	self bazCallback_PlayerConnect();
}
...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	//Lepko GunGame
	if( level.gg_knife_nerf && sMeansOfDeath == "MOD_MELEE" && sHitLoc != "head" && sHitLoc != "neck" && sHitLoc != "helmet" )
		iDamage = int( iDamage / 2 );

	if( level.gg_deadly_snipers >= 1 &&
		( sWeapon == "m40a3_acog_mp" || sWeapon == "m40a3_mp" || sWeapon == "remington700_acog_mp" || sWeapon == "remington700_mp" ) &&
		( sHitLoc == "head" || sHitLoc == "helmet" || sHitLoc == "torso_upper" || sHitLoc == "torso_lower" ) )
		iDamage = int( iDamage * 3 );

	if( level.gg_deadly_snipers == 2 &&
		( sWeapon == "m21_acog_mp" || sWeapon == "m21_mp" || sWeapon == "barrett_acog_mp" || sWeapon == "barrett_mp" || sWeapon == "dragunov_acog_mp" || sWeapon == "dragunov_mp") &&
		( sHitLoc == "head" || sHitLoc == "helmet" || sHitLoc == "torso_upper" || sHitLoc == "torso_lower") )
		iDamage = int( iDamage * 3 );
	//End 
...
//			}
		...
		if ( level.teamBased && isPlayer( eAttacker ) && (self != eAttacker) && (self.pers["team"] == eAttacker.pers["team"]) )
		{
			...
			if ( level.friendlyfire == 0 || level.friendlyfire > 3 || level.friendlyfire < 0 ) // no one takes damage // Lepko fix for >3 and <0
			{
				setdvar("scr_team_fftype", 0); //Lepko
				level.friendlyfire = 0; //Lepko
		...
		if ( isdefined(eAttacker) && eAttacker != self )
		{
			...
			if ( iDamage > 0 && level.mw_hitblip == 1 )
				eAttacker thread maps\mp\gametypes\_damagefeedback::updateDamageFeedback( hasBodyArmor );
			else if ( iDamage > 0 && level.mw_hitblip == 2 && !(iDFlags & level.iDFLAGS_PENETRATION) )
				eAttacker thread maps\mp\gametypes\_damagefeedback::updateDamageFeedback( hasBodyArmor );
		}
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if ( !level.inGracePeriod && level.gg_weapon_drop )
	{
		self maps\mp\gametypes\_weapons::dropWeaponForDeath( attacker );
		self maps\mp\gametypes\_weapons::dropOffhand();
	}

	self thread gungame\event::Killed_Player( attacker, sWeapon, sMeansOfDeath );/* GunGame [OneLine] */
